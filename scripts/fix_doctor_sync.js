...
//      to the correct active Doctor (matched by User.doctorId chain)
//   3. Delete orphaned Doctor records that no longer have any references
//
...
async function main() {
    ...
    if (orphaned.length === 0) {
        ...
        for (const orphan of orphaned) {
            ...
            // Try to find an active Doctor with matching name
            const nameLower = orphan.name.trim().toLowerCase();
            const replacement = allDoctors.find(d =>
                d.id !== orphan.id &&
                d.users.length > 0 &&
                d.users.some(u => u.isActive) &&
                (
                    d.name.trim().toLowerCase() === nameLower ||
                    d.name.trim().toLowerCase().includes(nameLower) ||
                    nameLower.includes(d.name.trim().toLowerCase())
                )
            );

            if (orphan._count.appointments === 0 && orphan._count.schedules === 0) {
                // Safe to delete — no references
                console.log(`    → Sin referencias. Se puede eliminar directamente.`);
                if (!DRY_RUN) {
                ...
                console.log(`    → Reemplazar por: "${replacement.name}" (id: ${replacement.id})`);
                if (!DRY_RUN) {
                ...
                // Has references but no clear replacement — manual review needed
                console.log(`    ⚠️  Tiene referencias pero no se encontró reemplazo claro.`);
                console.log(`    → ACCIÓN MANUAL REQUERIDA: revisar citas/horarios de este doctor.`);
            }
        }
        console.log();
    }

    // ─── STEP 3: Create missing Doctor records for doctor-users ───────────────
    ...
    console.log('=== FIN ===');
    if (DRY_RUN) {
        console.log('\nNingún cambio aplicado. Para aplicar, ejecutar:');
