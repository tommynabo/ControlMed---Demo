...
// Helper: Calculate total applying per-item discounts first, then commission, then global discount
const calculateBudgetTotal = (items, discountPercent = 0, commissionPercent = 0) => {
    // Per-item discounts applied at line level, commission is a global multiplier on the sum
    const subtotalWithItemDiscounts = items.reduce((sum, item) => {
        const price = Number(item.price) || 0;
        const qty = Number(item.quantity) || 1;
        const itemDiscount = Number(item.discount) || 0;
        return sum + price * (1 - itemDiscount / 100) * qty;
    }, 0);
    const withCommission = subtotalWithItemDiscounts * (1 + commissionPercent / 100);
