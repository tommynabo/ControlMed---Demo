...
async function createInvoice(contactId, items, date, dueDate, paymentMethod = 'cash', filingNumber = null) {
    ...
    // Quipu requires filing_number whenever issue_date is provided (API v1 constraint).
    // filingNumber must be pre-generated by the caller (sequential from local DB).
    ...
    if (date) {
        ...
        invoiceAttributes.paid_at       = date;
        invoiceAttributes.filing_number = filingNumber || `F-${new Date().getFullYear()}-${Date.now()}`;
    }

    // Confirmed by live API tests: Quipu accepts items inside relationships.items.data
    // as long as filing_number is also present in attributes. The `included` approach
    // creates the invoice but silently discards all items (total_amount stays 0.0).
    const payload = {
        data: {
            type: 'invoices',
